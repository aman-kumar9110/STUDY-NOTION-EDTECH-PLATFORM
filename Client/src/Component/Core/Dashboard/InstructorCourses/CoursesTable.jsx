...
export default function CoursesTable({ courses, setCourses }) {
  ...
  const handleCourseDelete = async (courseId) => {
    console.log("Delete triggered for:", courseId)
  setLoading(true)
                ...
                <Td colSpan={1} className="flex flex-1 gap-x-4 p-3">
                  <img
                    ...
                    className="md:h-[148px] md:w-[220px] aspect-video rounded-lg object-cover"
                     onClick={() => navigate(`/courses/${course._id}`)}   // ✅ navigate added
                  />
                  <div className="flex flex-col gap-1 justify-between">
                   <p
                     className="text-lg font-semibold text-richblack-5 mt-3 uppercase truncate tracking-wide cursor-pointer hover:text-yellow-100"
                        onClick={() => navigate(`/courses/${course._id}`)}   // ✅ navigate added
                         >
                    {course.courseName}
                          </p>
